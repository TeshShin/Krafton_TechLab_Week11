...
| **성능 측정(Stat)** | `GPUTimer` 기반 프로파일 구현. 스톨을 피하려고 쿼리 8개를 돌려쓰며 N-7 프레임 결과를 읽는 구조라, **스키닝 모드를 전환하면 이전 모드의 측정값이 보이던 것**을 그 지연에 맞춰 동기화. 모든 뷰의 스키닝 성능을 측정해 CPU/GPU 모드를 스왑 표시 |
